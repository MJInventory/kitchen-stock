export function createAppUserApi({
  ...
  return async function handleAppUserApi(req, res) {
    function sanitizeManagedUserPayload(actor, payload = {}) {
      const next = { ...payload };
      if (!actor?.permissions?.canManageAdminRoles) {
        delete next.desktopIdleTimeoutEnabled;
        delete next.blockedGotoMenu;
        delete next.blockedBackofficeMenu;
      }
      return next;
    }
    ...
    if (req.method === "POST" && req.url === "/api/app-users") {
      ...
      if (!user) return true;
      if (!requireRole(user, res, (candidate) => candidate.permissions.canAdminUsers, "Only admins can manage users.")) return true;
      const payload = sanitizeManagedUserPayload(user, await readJson(req));
