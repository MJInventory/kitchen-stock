...
export function createInternalDataDomain({
  ...
  function auditSnapshot(service) {
    ...
    return {
      ...service,
      password: service.password ? "[hidden]" : ""
    };
  }
  ...
  async function getInternalDataServiceDetail(recordId) {
    assertPostgresSchemaReady();
  ...
  async function listInternalDataServicesForExport() {
    assertPostgresSchemaReady();
    const result = await db().query(`
      select *
      from internal_data_services
  ...
  async function saveInternalDataService(payload, recordId = "", actorUsername = "") {
    ...
    if (recordId) {
      if (!isValidId(recordId)) throw new Error("Invalid internal data record.");
      const before = await getInternalDataServiceDetail(recordId);
      const result = await db().query(`
      ...
      if (!result.rows[0]) throw new Error("Internal data record was not found.");
      const saved = mapDetailRow(result.rows[0]);
      if (auditChanged?.(before, saved)) {
        await pgRecordAuditEntry?.({
          actionType: "change",
          entityType: "internal-data-service",
          entityId: saved.id,
          entityName: saved.serviceName,
          actorUsername,
          reasonCode: "internal-data-update",
    ...
    const saved = mapDetailRow(created.rows[0]);
    await pgRecordAuditEntry?.({
      actionType: "add",
      entityType: "internal-data-service",
      entityId: saved.id,
      entityName: saved.serviceName,
      actorUsername,
      reasonCode: "internal-data-create",
      after: auditSnapshot(saved)
    });
    return saved;
  }

  async function deleteInternalDataService(recordId, actorUsername = "") {
    assertPostgresSchemaReady();
    if (!isValidId(recordId)) throw new Error("Invalid internal data record.");
    const before = await getInternalDataServiceDetail(recordId);
    const result = await db().query(`
    ...
    await pgRecordAuditEntry?.({
      ...
      entityType: "internal-data-service",
      entityId: before.id,
      entityName: before.serviceName,
      actorUsername,
      reasonCode: "internal-data-delete",
      before: auditSnapshot(before)
    });
