export function createInventoryDomain({
  ...
  async function pgSaveSupplierPrice(itemId, supplierId, unitPrice, actorUsername = "") {
    if (!isValidId(supplierId)) return;
    await db().query(`
      ...
      on conflict (inventory_item_id, supplier_id) do update
        set unit_price = excluded.unit_price,
            ...
            updated_at = now()
    `, [itemId, supplierId, unitPrice, actorUsername]);
  }
  ...
  async function pgListItems() {
    const result = await db().query(`
      select
        ...
        coalesce(isp.unit_price, i.unit_price, 0) as unit_price,
        coalesce((
          select json_agg(
            json_build_object(
              'supplierId', prices.supplier_id,
              'unitPrice', prices.unit_price
            ) order by prices.supplier_id
          )
          from inventory_item_supplier_prices prices
          where prices.inventory_item_id = i.id
        ), '[]'::json) as supplier_prices
  ...
  async function pgUpdateItemSettings(recordId, payload, actorUsername = "") {
    ...
    await pgSaveSupplierPrice(recordId, supplierId, unitPrice, actorUsername);
  ...
  async function pgCreateInventoryItem(payload, actorUsername = "") {
    ...
    await pgSaveSupplierPrice(result.rows[0].id, supplierId, unitPrice, actorUsername);
    cache.items.expiresAt = 0;
    const items = await pgListItems();
