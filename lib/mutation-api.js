export function createMutationApi({
  ...
  function readUnitPrice(payload = {}) {
    return payload?.unitPrice ?? payload?.receivedUnitPrice ?? payload?.price;
  }
  ...
  return async function handleMutationApi(req, res) {
    ...
    if (req.method === "POST" && req.url.startsWith("/api/requests/") && req.url.endsWith("/receive")) {
      ...
      const request = await deliverRequest(recordId, user.name, {
        ...
        unitPrice: readUnitPrice(payload)
      });
      send(res, 200, { request });
      return true;
    }

    if (req.method === "PATCH" && req.url.startsWith("/api/requests/") && req.url.endsWith("/unit-price")) {
      const user = requireUser(req, res);
      if (!user) return true;
      const recordId = req.url.split("/")[3];
      const payload = await readJson(req);
      const request = await updateRequestUnitPrice(recordId, readUnitPrice(payload), user.name);
      send(res, 200, { request });
      return true;
    }
