export function createOperationsApi({
  ...
  saveDailyGuestCount,
  getManagementReport,
  searchManagementItems,
  getManagementItemHistory,
  listKitchenRoster,
  saveKitchenRoster,
  ...
  return async function handleOperationsApi(req, res) {
    ...
    if (req.method === "GET" && req.url.startsWith("/api/management-item-search")) {
      const user = requireUser(req, res);
      if (!user) return true;
      if (!requireRole(user, res, (candidate) => candidate.permissions.canAddInventoryItems, "Only inventory admins can use item history.")) return true;
      const url = new URL(req.url, "http://localhost");
      send(res, 200, { items: await searchManagementItems(url.searchParams.get("q")) });
      return true;
    }

    if (req.method === "GET" && req.url.startsWith("/api/management-item-history")) {
      const user = requireUser(req, res);
      if (!user) return true;
      if (!requireRole(user, res, (candidate) => candidate.permissions.canAddInventoryItems, "Only inventory admins can use item history.")) return true;
      const url = new URL(req.url, "http://localhost");
      send(res, 200, await getManagementItemHistory(url.searchParams.get("itemId")));
      return true;
    }
    ...
    if (req.method === "POST" && req.url.startsWith("/api/internal-data-services/") && req.url.endsWith("/details")) {
      const user = requireUser(req, res);
      if (!user) return true;
      if (!requireRole(user, res, (candidate) => candidate.permissions.canViewInternalData, "Only Security Admin users can view internal data.")) return true;
      const payload = await readJson(req);
      const currentUser = await findAppUserByName(user.name);
      const password = String(payload.password || "").trim();
      const validPassword = currentUser?.passwordHash
        ? await bcrypt.compare(password, currentUser.passwordHash)
        : String(currentUser?.password || "") === password;
      if (!validPassword) {
        send(res, 403, { error: "Login details were not correct." });
        return true;
      }
      const recordId = req.url.split("/")[3];
      send(res, 200, { service: await getInternalDataServiceDetail(recordId) });
      return true;
    }

    if (req.method === "POST" && req.url === "/api/internal-data-services/export") {
      const user = requireUser(req, res);
      if (!user) return true;
      if (!requireRole(user, res, (candidate) => candidate.permissions.canViewInternalData, "Only Security Admin users can export internal data.")) return true;
      const payload = await readJson(req);
      const currentUser = await findAppUserByName(user.name);
      const password = String(payload.password || "").trim();
      const validPassword = currentUser?.passwordHash
      ...
      send(res, 200, { services: await listInternalDataServicesForExport() });
      return true;
    }
    ...
    if (req.method === "DELETE" && req.url.startsWith("/api/internal-data-services/")) {
      ...
      if (!requireRole(user, res, (candidate) => candidate.permissions.canViewInternalData, "Only Security Admin users can manage internal data.")) return true;
      const recordId = req.url.split("/")[3];
      send(res, 200, { result: await deleteInternalDataService(recordId, user.name || "") });
      return true;
    }
