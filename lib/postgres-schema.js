...
export async function ensurePostgresSchemaUpgrades({ hasPostgres, db }) {
  ...
  postgresSchemaReady = (async () => {
    ...
    `);
    await db().query(`
      insert into kitchen_shift_types (shift_code, label, color, sort_order, active)
      values
        ('10_18', '10:00 - 18:00', '#fff1b8', 10, true),
        ('14_23', '14:00 - 23:00', '#c7f3f8', 20, true),
        ('14_22', '14:00 - 22:00', '#c7f9d4', 30, true),
        ('16_23', '16:00 - 23:00', '#d9ecff', 40, true),
        ('15_23', '15:00 - 23:00', '#ffd9df', 50, true),
        ('10_17', '10:00 - 17:00', '#e5ffc7', 60, true),
        ('NON', 'NON', '#ffe8c7', 70, true),
        ('VACATION', 'VACATION', '#fff1b8', 80, true),
        ('OFF', 'OFF', '#20242c', 90, true)
    ...
    `);
    await db().query(`
      create index if not exists idx_kitchen_roster_shifts_week_staff
        on kitchen_roster_shifts (week_start, user_id, shift_date)
    `);
    await db().query(`
      ...
      select
        ...
        case coalesce(nullif(kitchen_function, ''), 'Other')
          ...
          when 'Dishwasher' then 50
          when 'Pickup Waiter' then 60
          else 90
    ...
    `);
    await db().query(`
      drop view if exists order_report_summary_vw
    `);
    await db().query(`
      drop view if exists audit_daily_summary_vw
    `);
    await db().query(`
      drop view if exists driver_sheet_request_vw
    `);
    await db().query(`
      drop view if exists management_order_item_totals_vw
    `);
    await db().query(`
      drop view if exists management_order_summary_vw
    `);
    await db().query(`
    ...
    `);
    await db().query(`
      drop view if exists internal_order_details_vw
    `);
    await db().query(`
