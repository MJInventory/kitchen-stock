export function createRequestDomain({
  ...
  async function pgDeliverStandingOrderRunLine(recordId, userName, options = {}) {
    ...
    if (!requestId) throw new Error("This standing-order line is missing its linked order request.");
    return pgDeliverRequest(requestId, userName, options);
  }
  ...
  async function pgDeleteStandingOrderRunLine(recordId, actorUsername = "") {
    ...
    const requestId = String(line.resolved_request_id || "").trim();
    if (requestId) {
      return pgDeleteRequest(requestId, actorUsername);
    }
  ...
  async function pgRestoreDeletedRequestFromAudit(auditId, actorUsername = "") {
    ...
    if (!Number.isFinite(numericAuditId) || numericAuditId <= 0) {
    ...
    try {
      ...
      await pgRecordAuditEntry({
        ...
        note: `Deleted order restored from audit entry ${numericAuditId}.`,
        before: null,
        after: restored
      });
