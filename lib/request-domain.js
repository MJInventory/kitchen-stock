export function createRequestDomain({
  ...
  async function pgSaveSupplierPriceTx(client, inventoryItemId, supplierId, unitPrice, userName) {
    if (!isValidId(supplierId)) {
      await client.query(`
        update inventory_items
        set unit_price = $2, updated_at = now()
        where id = $1
      `, [inventoryItemId, unitPrice]);
      return;
    }
    await client.query(`
      insert into inventory_item_supplier_prices (
        inventory_item_id, supplier_id, unit_price, updated_by_username
      ) values ($1, $2, $3, $4)
      on conflict (inventory_item_id, supplier_id) do update
        set unit_price = excluded.unit_price,
            updated_by_username = excluded.updated_by_username,
            updated_at = now()
    `, [inventoryItemId, supplierId, unitPrice, userName]);
    await client.query(`
      update inventory_items
      set unit_price = case when primary_supplier_id = $2 then $3 else unit_price end,
          updated_at = now()
      where id = $1
    `, [inventoryItemId, supplierId, unitPrice]);
  }
  ...
  async function pgUpdateRequestUnitPrice(recordId, value, actorUsername = "") {
    if (!isValidId(recordId)) throw new Error("Invalid request record.");
    const unitPrice = Number(value);
    if (!Number.isFinite(unitPrice) || unitPrice < 0) {
      throw new Error("Price per unit must be zero or greater.");
    }
    const client = await db().connect();
    try {
      await client.query("begin");
        ...
        select r.id,
               ...
               coalesce(
                 (select d.supplier_id
                  ...
                  limit 1),
                 (select supply.resolved_supplier_id
                  from order_request_supply_vw supply
                  where supply.id = r.id),
      ...
      await pgRecordAuditEntry({
        ...
        actorUsername,
        reasonCode: "received-price-update",
        note: "Supplier price per unit updated from the receiving screen.",
        before: { unitPrice: Number(request.unit_price || 0) },
        after: { unitPrice }
      });
      return { id: recordId, unitPrice };
    } catch (error) {
      await client.query("rollback");
      throw error;
    } finally {
  ...
  async function pgDeliverRequest(recordId, userName, options = {}) {
    ...
    try {
      ...
      const receivedUnitPrice = rawUnitPrice === undefined || rawUnitPrice === null || rawUnitPrice === ""
        ? Number(request.current_unit_price || 0)
        : Number(rawUnitPrice);
      ...
      await pgSaveSupplierPriceTx(client, request.inventory_item_id, request.supplier_id, receivedUnitPrice, userName);
      await client.query(`
        update order_requests
        set unit_price = $2, updated_at = now()
        where id = $1
      `, [recordId, receivedUnitPrice]);
      ...
      await pgRecordAuditEntry({
        ...
        note: isFullReceipt
          ? `Order fully received. ${receivedQty} ${request.unit || ""} added to stock. Price ${receivedUnitPrice.toFixed(2)} saved.`
          : `Order partially received. ${receivedQty} ${request.unit || ""} added, ${remainingQty} ${request.unit || ""} still open. Price ${receivedUnitPrice.toFixed(2)} saved.`,
        before,
        after: latest
      });
      return {
  ...
  async function pgFindStandingOrderRunLine(recordId) {
    ...
    const result = await db().query(`
      select
        ...
        i.current_quantity,
        coalesce(
          (select s.id from suppliers s where lower(s.name) = lower(sorl.supplier_name) limit 1),
          (select so.supplier_id from standing_orders so where so.id = sorl.standing_order_id),
          i.primary_supplier_id
        ) as supplier_id,
        coalesce(
          (
            ...
            where price.inventory_item_id = sorl.inventory_item_id
              and price.supplier_id = coalesce(
                (select s.id from suppliers s where lower(s.name) = lower(sorl.supplier_name) limit 1),
                (select so.supplier_id from standing_orders so where so.id = sorl.standing_order_id),
                i.primary_supplier_id
              )
          ...
          case
            when coalesce(
              ...
              (select so.supplier_id from standing_orders so where so.id = sorl.standing_order_id),
              i.primary_supplier_id
            ) = i.primary_supplier_id then i.unit_price
            else 0
          end,
          0
        ) as current_unit_price,
  ...
  async function pgDeliverStandingOrderRunLine(recordId, userName, options = {}) {
    ...
    const rawUnitPrice = options.unitPrice ?? options.receivedUnitPrice ?? options.price;
    const receivedUnitPrice = rawUnitPrice === undefined || rawUnitPrice === null || rawUnitPrice === ""
      ? Number(line.current_unit_price || 0)
      : Number(rawUnitPrice);
    const receivedQty = Number(rawReceivedQty === undefined || rawReceivedQty === null || rawReceivedQty === "" ? orderedQty : rawReceivedQty);
    if (!Number.isFinite(receivedQty) || receivedQty <= 0) {
      throw new Error("Received quantity must be greater than zero.");
    ...
    try {
      ...
      await pgSaveSupplierPriceTx(client, line.inventory_item_id, line.supplier_id, receivedUnitPrice, userName);
      if (requestId) {
        await client.query(`
          update order_requests
          set unit_price = $2, updated_at = now()
          where id = $1
        `, [requestId, receivedUnitPrice]);
      }
