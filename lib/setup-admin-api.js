export function createSetupAdminApi({
  ...
  const adminGuard = "Only admins can manage units of measure.";
  ...
  return async function handleSetupAdminApi(req, res) {
    ...
    if (req.method === "GET" && req.url.startsWith("/api/setup/units-of-measure")) {
      const user = requireUser(req, res);
      if (!user) return true;
      if (!requireRole(user, res, (candidate) => candidate.permissions.canAdminUsers, adminGuard)) return true;
      send(res, 200, { units: await listUnitsOfMeasureAdmin() });
      return true;
    }
    ...
    if (req.method === "POST" && req.url === "/api/setup/units-of-measure") {
      const user = requireUser(req, res);
      if (!user) return true;
      if (!requireRole(user, res, (candidate) => candidate.permissions.canAdminUsers, adminGuard)) return true;
      const unit = await saveUnitOfMeasure(await readJson(req), "", user.name);
      send(res, 201, { unit });
      return true;
    }
    ...
    if (req.method === "PATCH" && req.url.startsWith("/api/setup/units-of-measure/")) {
      const user = requireUser(req, res);
      if (!user) return true;
      if (!requireRole(user, res, (candidate) => candidate.permissions.canAdminUsers, adminGuard)) return true;
      const recordId = req.url.split("/")[4];
      const unit = await saveUnitOfMeasure(await readJson(req), recordId, user.name);
      send(res, 200, { unit });
      return true;
    }
    ...
    if (req.method === "DELETE" && req.url.startsWith("/api/setup/units-of-measure/")) {
      const user = requireUser(req, res);
      if (!user) return true;
      if (!requireRole(user, res, (candidate) => candidate.permissions.canAdminUsers, adminGuard)) return true;
      const recordId = req.url.split("/")[4];
      const result = await deleteUnitOfMeasure(recordId, user.name);
      send(res, 200, { result });
      return true;
    }
