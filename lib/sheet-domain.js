export function createSheetDomain({
  ...
  async function pgEnsureDriverSheetLines(selectedDate) {
    ...
    await db().query(`
      ...
      set supplier_id = r.resolved_supplier_id,
          updated_at = now()
      from order_request_supply_vw r
      where d.order_request_id = r.id
        ...
        and r.status in ('Pending', 'Approved')
        and r.resolved_supplier_id is not null
        and d.supplier_id is distinct from r.resolved_supplier_id
  ...
  async function pgDriverSheetRequests(selectedDate, options = {}) {
    const includeStandingOrders = options.includeStandingOrders !== false;
    await pgEnsureDriverSheetLines(selectedDate);
    const result = await db().query(`
      select *
      from driver_sheet_request_vw
      where sheet_date = $1::text
        and delivered = false
        and status in ('Pending', 'Approved')
        and (requested_at at time zone $2)::date <= $1::date
        and ($3::boolean = true or coalesce(is_standing_order, false) = false)
      order by supplier_name nulls last, category nulls last, shelf_code nulls last, item_name
    `, [selectedDate, appTimeZone, includeStandingOrders]);
    return result.rows.map((row) => pgRequestFromRow({
      ...row,
      id: row.request_id,
      supplier_name: row.supplier_name,
      supplier_contact: row.supplier_contact,
      driver_line_id: row.id,
      ordered: row.ordered,
      ordered_at: row.ordered_at,
      ordered_by: row.ordered_by_username,
      ...
      received: row.received || row.delivered,
      received_at: row.received_at || row.delivered_at,
      received_by: row.received_by_username || row.delivered_by
    }));
  }
  ...
  async function pgListDriverSheet(date) {
    ...
    const [requests, suppliers, supplierNotes] = await Promise.all([
      pgDriverSheetRequests(selectedDate, { includeStandingOrders: false }),
      pgListSuppliers(),
      pgListSupplierDeliveryNotes(selectedDate)
    ]);
    const driverName = await pgResolveDriverName(selectedDate);
    return { date: selectedDate, driverName, requests, suppliers, supplierNotes };
  }
  ...
  async function pgListOrderReport(date) {
    const selectedDate = /^\d{4}-\d{2}-\d{2}$/.test(date || "") ? date : todayIso();
    await pgEnsureDriverSheetLines(selectedDate);
    const [guestCount, standingOrders, activity, activitySummary] = await Promise.all([
      pgGetDailyGuestCount(selectedDate),
      pgListStandingOrders({ activeOnly: true }),
      pgListAuditEntries(selectedDate),
      pgGetAuditSummary(selectedDate)
    ]);
    const result = await db().query(`
      select *
      from driver_sheet_request_vw
      where sheet_date = $1::text
        and coalesce(is_standing_order, false) = false
      order by supplier_name nulls last, category nulls last, item_name
    `, [selectedDate]);
    const summaryResult = await db().query(`
      select *
      from order_report_summary_vw
      where sheet_date = $1::text
    `, [selectedDate]);
    ...
    return {
      ...
      summary: {
        ...
        orderedLines: Number(summaryRow.ordered_lines || 0),
        deliveredLines: Number(summaryRow.delivered_lines || 0),
        waitingLines: Number(summaryRow.waiting_lines || 0),
        toDeliverLines: Number(summaryRow.to_deliver_lines || 0),
        deliveredValue: Number(summaryRow.delivered_value || 0)
      },
      guestCount,
      rows,
      standingOrders,
  ...
  async function pgUpdateDriverLine(recordId, payload, userName) {
    ...
    const currentResult = await db().query(`
      select d.id, d.sheet_date::text as sheet_date, d.order_request_id, r.inventory_item_id,
             r.standing_order_run_line_id, sp.id as current_supplier_id, d.to_deliver, d.ordered
      from driver_sheet_lines d
    ...
    if (Object.prototype.hasOwnProperty.call(payload, "supplierName")) {
      ...
      if (payload.updatePrimarySupplier) {
        ...
        await db().query(`
          ...
          set primary_supplier_id = $2,
              unit_price = coalesce((
                select price.unit_price
                from inventory_item_supplier_prices price
                where price.inventory_item_id = item.id and price.supplier_id = $2
              ), 0),
              updated_at = now()
          where item.id = $1
        `, [current.inventory_item_id, supplier.id]);
    ...
    if (payload.ordered === true || payload.toDeliver === true || (Object.prototype.hasOwnProperty.call(payload, "supplierName") && current.ordered)) {
      await db().query(`
        update order_requests r
        set unit_price = coalesce(
              (
                select price.unit_price
                from driver_sheet_lines line
                join inventory_item_supplier_prices price
                  on price.inventory_item_id = r.inventory_item_id
                 and price.supplier_id = line.supplier_id
                where line.id = $2
              ),
              (select item.unit_price from inventory_items item where item.id = r.inventory_item_id),
              0
            ),
            updated_at = now()
        where r.id = $1
      `, [current.order_request_id, recordId]);
    }
  ...
  async function pgDeliverDriverLine(recordId, requestRecordId, userName, options = {}) {
    ...
    const lineCheck = await db().query(`
      ...
      from driver_sheet_lines
      where id = $1
      limit 1
    `, [recordId]);
    const line = lineCheck.rows[0] || null;
    if (!line) throw new Error("Driver line was not found. Reload the driver sheet and try again.");
    if (String(line.order_request_id || "") !== String(requestRecordId || "")) {
      throw new Error("Driver line and order request do not match. Reload the driver sheet and try again.");
    }
    ...
    if (request.fullyDelivered) {
      await db().query(`
        ...
        where id = $1 and order_request_id = $3
      `, [recordId, userName, requestRecordId]);
    } else {
      await db().query(`
        update driver_sheet_lines
        set received = false, received_at = null, received_by_username = '', updated_at = now()
        where id = $1 and order_request_id = $2
