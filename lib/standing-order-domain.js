export function createStandingOrderDomain({
  ...
  async function pgListStandingOrders(options = {}) {
    ...
    const result = await db().query(`
        ...
        coalesce(
          json_agg(
            json_build_object(
              ...
              'unit', u.name,
              'unitPrice', coalesce(isp.unit_price, case when source_order.supplier_id = i.primary_supplier_id then i.unit_price else 0 end, 0),
              'lineTotal', soi.quantity * coalesce(isp.unit_price, case when source_order.supplier_id = i.primary_supplier_id then i.unit_price else 0 end, 0)
            )
            order by i.name
      ...
      left join standing_orders source_order on source_order.id = so.id
      ...
      left join inventory_item_supplier_prices isp
       ...
       and isp.supplier_id = source_order.supplier_id
      where coalesce(so.deleted, false) = false
        and (
          $1::boolean = false
          or (
