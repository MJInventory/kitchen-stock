...
import { appBuildId, appReleaseLabel, appVersion } from "./app-config.js";
...
export function createViewHelpers(viewsDir) {
  function assetWithVersion(src = "") {
    ...
    return `${value}${separator}v=${encodeURIComponent(appBuildId)}`;
  }
  ...
  function pageScripts(moduleSrc, { offline = false } = {}) {
    const scripts = ["/menu-config.js", "/screen-access.js", "/menus.js", "/push.js", "/theme.js", "/pwa-install.js"].map(assetWithVersion);
