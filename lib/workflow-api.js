export function createWorkflowApi({
  ...
  return async function handleWorkflowApi(req, res) {
    ...
    if (req.method === "POST" && req.url === "/api/standing-orders/sync") {
      const user = requireUser(req, res);
      if (!user) return true;
      if (!requireRole(user, res, (candidate) => candidate.permissions.canAddInventoryItems, "Only admins and power users can sync standing orders.")) return true;
      const payload = await readJson(req);
      const result = await ensureStandingOrdersForDate(String(payload?.date || "").trim(), user.name || "System");
      send(res, 200, result);
      return true;
    }
